...
import { Sparkles, Search, Navigation } from 'lucide-react'
...
const ETAPrediction = () => {
  ...
  if (loading && selectedTrainNumber) {
    return <Loader message={`Computing AI arrival forecast for train #${selectedTrainNumber}...`} />
  }
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-slate-500 font-medium mt-0.5">
            Multi-factor AI arrival forecasting with dynamic recovery calibration
          </p>
        </div>
      ...
      {prediction ? (
        ...
        <div className="bg-white rounded-2xl p-10 border border-slate-200/90 shadow-xs text-center">
          ...
          <h3 className="text-base font-bold text-slate-900">
            No train selected for ETA prediction
          </h3>
          <p className="text-xs text-slate-500 font-medium mt-1 max-w-md mx-auto leading-relaxed">
            Search for a train number to generate AI-calibrated arrival times, corridor friction models, and confidence scoring.
          </p>
